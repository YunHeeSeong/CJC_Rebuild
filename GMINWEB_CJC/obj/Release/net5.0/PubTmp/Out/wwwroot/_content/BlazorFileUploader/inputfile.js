(function () {
    ...
    window.BlazorInputFile = {
        init: function init(elem, componentInstance) {
            ...
            elem.addEventListener('change', async function handleInputFileChange(event) {
                ...
                var fileList = Array.prototype.map.call(elem.files, function (file) {
                    ...
                    if (result.type.startsWith("video")) {
                        ...
                        tempVideoEl.src = window.URL.createObjectURL(file);

                        tempVideoEl.addEventListener('loadedmetadata', function () {
                            // duration is now available here -- store it somewhere as you like
                            result.videoDuration = Math.floor(tempVideoEl.duration / 60) + ':' + ('0' + Math.floor(tempVideoEl.duration % 60)).slice(-2)
                        });
                ...
                await sleep(1500); // Time to load the metadata to the video
